...
async def test_stream_yields_deltas_and_usage(mock_gemini_adapter: GeminiAdapter) -> None:
    chunks = [c async for c in mock_gemini_adapter.stream(_req())]
    assert "".join(c.delta for c in chunks) == "Hello from the Gemini mock."
    final = chunks[-1]
    assert final.finish_reason == "stop"
    assert final.usage is not None
    assert final.usage.prompt_tokens == 6
    # completion = total - prompt = 15 - 6
